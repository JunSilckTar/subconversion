...
pub fn create_folder(path: &str) -> io::Result<()> {
    ...
    while attempts < MAX_RETRIES {
        if !folder_path.exists() {
            info!("Folder {:?} does not exist. Creating...", folder_path);
            match fs::create_dir_all(folder_path) {
                ...
                Err(e) => {
                    error!("Failed to create folder {:?}: {}. Retrying...", folder_path, e);
                    attempts += 1;
                    sleep(RETRY_DELAY);
                }
            }
...
pub fn create_file(path: &str) -> io::Result<()> {
    ...
    if let Some(parent) = file_path.parent() {
        if !parent.exists() {
            info!("Folder {:?} does not exist. Creating...", parent);
    ...
    while attempts < MAX_RETRIES {
        if !file_path.exists() {
            info!("File {:?} does not exist. Creating...", file_path);
            match fs::File::create(file_path) {
                ...
                Err(e) => {
                    error!("Failed to create file {:?}: {}. Retrying...", file_path, e);
                    attempts += 1;
                    sleep(RETRY_DELAY);
                }
            }
